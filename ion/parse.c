...
Expr *parse_expr_operand() {
    if (is_token(TOKEN_INT)) {
        ...
        fatal_syntax_error("Unexpected token %s in expression", token_kind_str(token.kind));
        return NULL;
    }
}
...
Decl *parse_decl_var() {
    ...
    if (match_token('=')) {
        ...
        fatal_syntax_error("Expected : or = after var, got %s", token_kind_str(token.kind));
        return NULL;
    }
}
...
Decl *parse_decl() {
    if (match_keyword(enum_keyword)) {
        ...
        fatal_syntax_error("Expected declaration keyword, got %s", token_kind_str(token.kind));
        return NULL;
    }
}
