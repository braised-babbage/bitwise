...
Sym *sym_get(const char *name) {
    for (Sym *it = sym_list; it != buf_end(sym_list); it++) {
        if (it->name == name) {
            return it;
        }
    }
...
void sym_put(Decl *decl) {
    ...
    buf_push(sym_list, (Sym){decl->name, decl, SYM_UNRESOLVED});
}
...
void resolve_sym(Sym *sym) {
    ...
    if (sym->state == SYM_RESOLVING) {
        fatal("Cyclic dependency in symbol %s", sym->name);
        return;
    }
...
void resolve_test() {
    ...
    Type *int_ptr_ptr = type_ptr(int_ptr);
    assert(type_ptr(int_ptr) == int_ptr_ptr);
